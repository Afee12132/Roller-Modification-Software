...
####绘图函数###################################################
    def hui1():
        ...
        s1 = qjbl.get_v('De')[:,int((int(Ys.get())+1)/2)]
        a1.plot(s1)
        canvas1.show()
    ...
    l5 = Label(f1_1,text='滚子半锥角 θ',font=(10))
    l5.grid(row=5,column=0, padx=20,pady=8,sticky=W)
    θ = StringVar()
    θ.set(2)
    e5 = Entry(f1_1,textvariable=θ)
    e5.grid(row=5,column=1)
    l5_5 = Label(f1_1,text='°',font=(10))
...
# =============================================================================
#     l11 = Label(f1_1,text='圆弧半径 R',font=(10))
#     l11.grid(row=11,column=0, padx=20,pady=8,sticky=W)
#     R = StringVar()
#     R.set(900)
#     e11 = Entry(f1_1,textvariable=R)
#     e11.grid(row=11,column=1)
#     l11_11 = Label(f1_1,text='mm',font=(10))
#     l11_11.grid(row=11,column=2,pady=8,sticky=W)
# =============================================================================
    
    
    b = Button(f1_1,text='开始计算',command=lambda :yzhuids.fun(float(cd.get()),float(D.get()),float(d.get()),float(yx.get()),float(θ.get()),float(E.get())
                  ,float(mo.get()),int(Xs.get()),int(Ys.get()),float(F.get()),f1,canvas1,f2,canvas2),fg='red')
    b.grid(row=12,column=0,columnspan=2,padx=30,pady=5,ipadx=25,)
