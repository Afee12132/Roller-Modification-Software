...
def fun(cd,D,d,yx,θ,E,mo,Xs,Ys,F,Rs,Pl,ff1,canvas1,ff2,canvas2):
    def a_ij(x,y,b,a):
    ...
    def yuanjiao(x_n,dx,r):
        ds =  r - np.sqrt(r**2 - dx**2)
        R_r = Ri(x_n) - ds
        return R_r
    ...
    for i in range(x_grid_number):
        for j in range(y_grid_number):
            if np.logical_and(np.logical_and(xa[i] <= pl/2 , xa[i] >= -pl/2), np.logical_and(ya[j]<y2, ya[j] > -y2)):
                z[i,j] = np.sqrt(Ri(xa[i])**2 - ya[j]**2)
            if np.logical_and(np.logical_and(xa[i] < -pl/2 , xa[i] > -x_yx), np.logical_and(ya[j]<y2, ya[j] > -y2)):
                R_r=yuanjiao(xa[i],-pl/2-xa[i],Rs)
                z[i,j] = np.sqrt(R_r**2 - ya[j]**2)
            if np.logical_and(np.logical_and(xa[i] < x_yx, xa[i] > pl/2), np.logical_and(ya[j]<y2, ya[j] > -y2)):
                R_r=yuanjiao(xa[i],xa[i]-pl/2,Rs)
                z[i,j] = np.sqrt(R_r**2 - ya[j]**2)
...
# =============================================================================
    ...
    qjbl.set_v('Dw',c.reshape(x_grid_number, y_grid_number))
        ###图1
    ff1.clf()
    a1 = ff1.add_subplot(111)
    s1 = qjbl.get_v('Dw')[:,int((y_grid_number+1)/2)]
    a1.plot(s1)
    canvas1.show()
