...
#           (i-k+0.5) * np.log(f1(i-k+0.5, j-l+0.5)/f1(i-k-0.5,j-l+0.5))
def fun(cd,Rb,yx,E,mo,Xs,Ys,F,Rr,ff1,canvas1,ff2,canvas2):
    def a_ij(x,y,b,a):
    ...
    def yuanjiao(x_n,x, r):
        ds = r - np.sqrt(r**2-x**2)
        R_r = R - ds 
        return R_r
    ...
    for i in range(x_grid_number):
        for j in range(y_grid_number):
            if np.logical_and(np.logical_and(xa[i] < x_yx/3 , xa[i] > -x_yx/3), np.logical_and(ya[j]<1, ya[j] > -1)):
                z[i,j] = np.sqrt(R**2 - ya[j]**2)
            if np.logical_and(np.logical_and(xa[i] < -x_yx/3, xa[i] > -x_yx), np.logical_and(ya[j]<1, ya[j] > -1)):
                R_r=yuanjiao(xa[i],xa[i]+x_yx/3,Rs)
                z[i,j] = np.sqrt(R_r**2 - ya[j]**2)
            if np.logical_and(np.logical_and(xa[i] < x_yx, xa[i] > x_yx/3), np.logical_and(ya[j]<1, ya[j] > -1)):
                R_r=yuanjiao(xa[i],xa[i]-x_yx/3,Rs)
                z[i,j] = np.sqrt(R_r**2 - ya[j]**2)
...
# =============================================================================
    ...
    qjbl.set_v('Ds',c.reshape(x_grid_number, y_grid_number))
    ##图1
    ff1.clf()
    a1 = ff1.add_subplot(111)
    s1 = qjbl.get_v('Ds')[:,int((y_grid_number+1)/2)]
    a1.plot(s1)
    canvas1.show()
