...
####绘图函数###################################################
    def hui1():
        ...
        s1 = qjbl.get_v('Ds')[:,int((int(Ys.get())+1)/2)]
        a1.plot(s1)
        canvas1.show()
    ...
    l11 = Label(f1_1,text='圆弧半径 Rr',font=(10))
    l11.grid(row=11,column=0,padx=20,pady=8,sticky=W)
    Rr = StringVar()
    Rr.set(900)
    e11 = Entry(f1_1,textvariable=Rr)
    e11.grid(row=11,column=1)
    l11_11 = Label(f1_1,text='mm',font=(10))
    l11_11.grid(row=11,column=2,pady=8,sticky=W)
    
    
    b = Button(f1_1,text='开始计算',command=lambda :yzy.fun(float(cd.get()),float(R.get()),float(yx.get()),float(E.get())
                  ,float(mo.get()),int(Xs.get()),int(Ys.get()),float(F.get()),float(Rr.get()),f1,canvas1,f2,canvas2),fg='red')
