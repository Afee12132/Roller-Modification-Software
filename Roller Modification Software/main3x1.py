...
####绘图函数###################################################
    ...
    b = Button(f1_1,text='开始计算',command=lambda :yzhuiz.fun(float(cd.get()),float(D.get()),float(d.get()),float(yx.get()),float(θ.get()),float(E.get())
    ...
    #########  绘图   ###################################
    f2_1 = Frame(Fr)
    f2_1.grid(row=0,column=1,pady=20,sticky=W+N,padx=30)
